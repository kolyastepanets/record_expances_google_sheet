  ...
  class Sainsburys < Default
    ...
    def group_texts_for_parsed_texts
      ...
      @parsed_texts.deep_dup.each.with_index do |array_of_text, index|
        break if sainsburys_end?(array_of_text)

        next if !array_of_text.any? { |str| str.downcase.include?('£') }
    ...
    def total_price_array_of_text
      @parsed_texts.detect { |array_of_text| sainsburys_end?(array_of_text) }
    end
    ...
    def sainsburys_end?(array_of_text)
