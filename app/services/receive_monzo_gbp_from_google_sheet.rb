class ReceiveMonzoGbpFromGoogleSheet < GetOrSetDataInGoogleSheetBase
  COLUMN_LETTER = ('A'..'G').to_a
  KEY_FIND_CELL_LEFT_WISE_MONEY = "GBP Monzo".freeze
  NEXT_SECOND_CELL = 2
  ...
  def parse_response
    ...
    {
      gbp_monzo_formula: gbp_monzo_formula,
      coordinates_of_gbp_monzo_formula: "#{COLUMN_LETTER[NEXT_SECOND_CELL]}#{start_line_to_search + index_line_to_remember}",
    }
  end
end
